...
export class MarkSenderSuspiciousUseCase {
  public constructor(
  ...
  public async execute(context: AuthenticatedContext, senderId: string): Promise<SenderProfile> {
    ...
    const updatedSender = await this.senders.update(sender.id, {
      status: "SUSPICIOUS",
      trustScore: Math.min(sender.trustScore, 35),
      riskScore: Math.max(sender.riskScore, 75),
    });
    ...
    await this.auditLogs.create({
      ...
      action: "SENDER_MARKED_SUSPICIOUS",
      entityType: "SenderProfile",
      entityId: sender.id,
      description: `Remitente ${sender.email} marcado como sospechoso.`,
      ip: null,
